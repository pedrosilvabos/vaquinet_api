...
async function isInsideFence({ farmId, lat, lon }) {
  ...
  const { data, error } = await supabase.rpc("is_inside_fence", {
    p_farm_id: farmId,
    p_lat: lat,
    p_lon: lon,
  });
...
export async function postBaseStatus(req, res) {
  ...
  const row = {
    ...
    status_data: {
      ...
      backhaul_name: normalizeOptionalText(
        payload.backhaul_name ?? payload.operator_name,
      ),
      backhaul_signal_percent:
        payload.backhaul_signal_percent ?? payload.signal_percent ?? null,
...
export async function batchTelemetry(req, res) {
  ...
  for (const node of items) {
    ...
    const normalizedBatteryVoltage =
      eventData?.node_battery_voltage ??
      eventData?.batteryVoltage ??
      eventData?.node_battery ??
      null;
    const normalizedVbus =
      eventData?.node_vbus ?? eventData?.vbusVoltage ?? null;
    ...
    try {
      ...
      console.log(
        "incoming node event_data keys:",
        ...new Set(items.flatMap((d) => Object.keys(d?.event_data || {}))),
      );
      ...
      if (existingNode) {
        ...
        if (updateError)
          console.warn(
            `⚠️ Failed to update node ${nodeId}:`,
            updateError.message,
          );
      } else {
        ...
        if (insertError)
          console.warn(
            `⚠️ Failed to insert node ${nodeId}:`,
            insertError.message,
          );
      }
      ...
      const eventPayload = {
        ...
        event_data: {
          ...
          node_gps_course:
            eventData.node_gps_course ?? eventData.node_gpsCourse ?? null,
          node_gps_altitude:
            eventData.node_gps_altitude ?? eventData.node_gpsAltitude ?? null,
          node_gps_speed:
            eventData.node_gps_speed ?? eventData.node_gpsSpeed ?? null,
          ...
          temperature:
            eventData.temperature ?? eventData.node_temperature ?? null,
          node_battery_voltage: normalizedBatteryVoltage,
          node_vbus: normalizedVbus,
          base_battery: eventData.base_battery ?? eventData.baseBattery ?? null,
          base_battery_percent:
            eventData.base_battery_percent ??
            eventData.baseBatteryPercent ??
            null,
          base_vbus: eventData.base_vbus ?? eventData.baseVbus ?? null,
          base_latitude:
            eventData.base_latitude ?? eventData.baseLatitude ?? null,
          base_longitude:
            eventData.base_longitude ?? eventData.baseLongitude ?? null,
          isAlerted: eventData.isAlerted ?? null,
          alertType: eventData.alertType ?? null,
          backhaul: eventData.backhaul ?? null,
          backhaul_name:
            eventData.backhaul_name ??
            eventData.operator_name ??
            eventData.operatorName ??
            null,
          rat_name: eventData.rat_name ?? eventData.ratName ?? null,
          backhaul_signal_percent:
            eventData.backhaul_signal_percent ??
            eventData.signal_percent ??
            eventData.signalPercent ??
            null,
          lte_signal_quality:
            eventData.lte_signal_quality ?? eventData.lteSignalQuality ?? null,
          lora_rssi: eventData.lora_rssi ?? null,
          lora_snr: eventData.lora_snr ?? null,
          gps_config_version_applied: normalizeOptionalInteger(
            eventData.gps_config_version_applied ??
              eventData.gpsConfigVersionApplied,
          ),
      ...
      try {
        ...
        if (behaviorResult?.ok === false) {
          ...
          behaviorResult.status !== "inserted" &&
          behaviorResult.reason !== "missing_motion_window"
        ) {
