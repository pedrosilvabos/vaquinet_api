...
export async function postBaseStatus(req, res) {
    ...
    const row = {
        ...
        status_data: {
            ...
            backhaul_name: normalizeOptionalText(payload.backhaul_name ?? payload.operator_name),
            backhaul_signal_percent: payload.backhaul_signal_percent ?? payload.signal_percent ?? null,
...
export async function batchTelemetry(req, res) {
    ...
    for (const node of data) {
        ...
        const normalizedBatteryVoltage = eventData?.node_battery_voltage ?? eventData?.batteryVoltage ?? eventData?.node_battery ?? null;
        const normalizedVbus = eventData?.node_vbus ?? eventData?.vbusVoltage ?? null;
        ...
        try {
         ...
         const eventPayload = {
          ...
          event_data: {
            ...
            node_gps_course: eventData.node_gps_course ?? eventData.node_gpsCourse ?? null,
            node_gps_altitude: eventData.node_gps_altitude ?? eventData.node_gpsAltitude ?? null,
            node_gps_speed: eventData.node_gps_speed ?? eventData.node_gpsSpeed ?? null,
            ...
            temperature: eventData.temperature ?? eventData.node_temperature ?? null,
            ...
            base_battery_percent: eventData.base_battery_percent ?? eventData.baseBatteryPercent ?? null,
            base_vbus: eventData.base_vbus ?? eventData.baseVbus ?? null,
            base_latitude: eventData.base_latitude ?? eventData.baseLatitude ?? null,
            base_longitude: eventData.base_longitude ?? eventData.baseLongitude ?? null,
            isAlerted: eventData.isAlerted ?? null,
            alertType: eventData.alertType ?? null,
            backhaul: eventData.backhaul ?? null,
            backhaul_name: eventData.backhaul_name ?? eventData.operator_name ?? eventData.operatorName ?? null,
            rat_name: eventData.rat_name ?? eventData.ratName ?? null,
            backhaul_signal_percent: eventData.backhaul_signal_percent ?? eventData.signal_percent ?? eventData.signalPercent ?? null,
